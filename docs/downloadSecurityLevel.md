...
| 11 | Fail-fast（可疑/不确定即失败）                             | **OFF** | 边界清晰：DNS 失败/缺 baseDir/非法 header 等直接拒绝，减少“模糊降级”带来的安全灰区             | 失败率上升；需要 UI/调用方解释更多错误；用户会觉得“浏览器能下你却不能”                                           | **8:6** 结论：安全更强但体验更硬。建议：安全红线场景开；面向普通用户的“能下就行”产品建议关。                 |
