...
// ★ 服务器下发的动态跳转 URL
// 新架构: links 字典（服务端可任意扩展 key，客户端按语义取值）
// 兼容旧架构: url_a → links.a, url_z → links.z
let _dynamicLinks = {};

/**
 * 更新服务器下发的动态 URL
 * @param {Object} linksObj - 服务器下发的 links 字典，如 { price: '...', profile: '...', a: '...', z: '...' }
 */
function setDynamicLinks(linksObj) {
	if (linksObj && typeof linksObj === 'object') {
		for (const [key, val] of Object.entries(linksObj)) {
			if (typeof val === 'string' && val) _dynamicLinks[key] = val;
		}
	}
}

/**
 * 获取动态 URL：优先服务器下发，兆底静态默认值
 * @param {string} key - 链接语义 key，如 'price', 'profile', 'a', 'z'
 * @param {string} [fallbackPath] - 兆底 gh555.com 路径
 * @param {string} [fallbackFragment] - 兆底 hash fragment
 */
function getDynamicUrl(key, fallbackPath, fallbackFragment) {
	const serverUrl = _dynamicLinks[key];
	const ide = getIDEFamily();
	const ver = getClientVersion();
	if (serverUrl) {
	...
	try {
		...
		if (data && data.ok) {
			return {
				...
				links: (data.links && typeof data.links === 'object') ? data.links : null,
		...
		if (data && typeof data.active_12h === 'number') {
			return { active_12h: data.active_12h, total_installations: null, total_companion_seconds: null, updated_at: null, url_a: null, url_z: null, links: null, ping_interval_s: null, radio_live: false, radio_m3u8: null, radio_stream: null, active_playing: null, popup: null };
		}
		return null;
	} catch (e) {
		return null;
