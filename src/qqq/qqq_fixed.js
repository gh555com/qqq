...
async function handleClipboardNode(targetDir, partialCallback = null, token = null, progressCallback = null) {
    try {
        ...
        const rootEl = $('body').length ? $('body').get(0) : $.root().get(0);

        if (!rootEl) return { type: "text", text: cleanText || "" };
...
async function deactivate() {
    ...
    try { pythonBridge?.stop?.(); } catch { }
    try { rustBridge?.stop?.(); } catch { }
    try { shellBridge?.stop?.(); } catch { }
